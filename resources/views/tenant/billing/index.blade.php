    ...
    <div class="bg-white shadow rounded-lg p-6 mb-8">
        ...
        <div class="mt-6 pt-6 border-t border-gray-200">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('tenant.billing.page') }}" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:opacity-90" style="background-color: var(--brand-primary);">
                    Upgrade Plan (Requires Payment)
                </a>
            </div>
            <p class="text-sm text-gray-600 mb-4">
                <strong>Note:</strong> To upgrade to a paid plan, please use the "Upgrade Plan" button above. Downgrades can be done below.
            </p>
            <form method="POST" action="{{ route('tenant.billing.update-plan') }}" id="update-plan-form">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <label class="flex items-center p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 {{ $subscription->plan === $planKey ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200' }}">
                        <input type="radio" name="plan" value="{{ $planKey }}" {{ $subscription->plan === $planKey ? 'checked' : '' }} class="mr-3 plan-radio">
                        <div>
                ...
                <div class="mt-4">
                    <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white" style="background-color: var(--brand-primary);">
                        Update Plan (Downgrade Only)
                    </button>
                    <p class="mt-2 text-sm text-gray-500">
                        This form is for downgrades only. Upgrades require payment and must be done through the upgrade page.
                    </p>
                </div>
            </form>
        </div>
